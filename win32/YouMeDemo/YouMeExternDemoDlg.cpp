...
enum pcmCallBackFlag {
	pcmCallBackFlag_Remote = 0x01,
	pcmCallBackFlag_Record = 0x02,
	pcmCallBackFlag_Mix = 0x04,
};

static ExternVideoProfile  videoProfileArray[22] = { { 1280, 720, 15 }, { 320, 240, 15 }, { 320, 240, 20 }, { 320, 240, 25 }, { 320, 240, 30 },
{ 640, 360, 15 }, { 640, 360, 30 },{ 640, 480, 15 }, { 640, 480, 20 }, { 640, 480, 25 }, { 640, 480, 30 },
{ 960, 720, 15 }, { 960, 720, 20 }, { 960, 720, 25 }, { 960, 720, 30 },
 { 1280, 720, 20 }, { 1280, 720, 25 }, { 1280, 720, 30 },
{ 1920, 1080, 15 }, { 1920, 1080, 20 }, { 1920, 1080, 25 }, { 1920, 1080, 30 },
};
...
CYouMeExternDemoDlg* YouMeExternDemoDlg = NULL;
class CExternAboutDlg : public CDialogEx
{
public:
	CExternAboutDlg();
...
CExternAboutDlg::CExternAboutDlg() : CDialogEx(CExternAboutDlg::IDD)
{
}

void CExternAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CExternAboutDlg, CDialogEx)
END_MESSAGE_MAP()


// CYouMeExternDemoDlg 对话框
CYouMeExternDemoDlg::CYouMeExternDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CYouMeExternDemoDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
	YouMeExternDemoDlg = this;
}

void CYouMeExternDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
void CYouMeExternDemoDlg::OnCancel()
{
	if (isInRoom) {
...
BEGIN_MESSAGE_MAP(CYouMeExternDemoDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	...
	ON_BN_CLICKED(IDC_BTN_LEAVEROOM, &CYouMeExternDemoDlg::OnBnClickedBtnLeaveroom)
	ON_BN_CLICKED(IDC_BTN_JOINROOM, &CYouMeExternDemoDlg::OnBnClickedBtnJoinroom)
	ON_BN_CLICKED(IDC_BTN_INIT, &CYouMeExternDemoDlg::OnBnClickedBtnInit)
	ON_MESSAGE(UM_CONTROL, &CYouMeExternDemoDlg::OnInterfaceEvent)
	//ON_MESSAGE(CALL_CONTROL, &CYouMeExternDemoDlg::OnControlCallMessage)
	ON_BN_CLICKED(IDC_BTN_UNINIT, &CYouMeExternDemoDlg::OnBnClickedBtnUninit)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_VOL, &CYouMeExternDemoDlg::OnNMCustomdrawSliderVol)
	ON_BN_CLICKED(IDC_BTN_CLOSE_MIC, &CYouMeExternDemoDlg::OnBnClickedBtnCloseMic)
	ON_BN_CLICKED(IDC_BTN_GET_MIC, &CYouMeExternDemoDlg::OnBnClickedBtnGetMic)
	ON_BN_CLICKED(IDC_BTN_OPEN_MIC, &CYouMeExternDemoDlg::OnBnClickedBtnOpenMic)
	ON_BN_CLICKED(IDC_BTN_GET_SPEAKER, &CYouMeExternDemoDlg::OnBnClickedBtnGetSpeaker)
	ON_BN_CLICKED(IDC_BTN_OPEN_SPEAKER, &CYouMeExternDemoDlg::OnBnClickedBtnOpenSpeaker)
	ON_BN_CLICKED(IDC_BTN_CLOSE_SPEAKER, &CYouMeExternDemoDlg::OnBnClickedBtnCloseSpeaker)
	ON_BN_CLICKED(IDC_BTN_OPEN_OTHER_SPEAKER, &CYouMeExternDemoDlg::OnBnClickedBtnOpenOtherSpeaker)
	ON_BN_CLICKED(IDC_BTN_CLOSE_OTHER_SPEAKER, &CYouMeExternDemoDlg::OnBnClickedBtnCloseOtherSpeaker)
	ON_BN_CLICKED(IDC_BTN_CLOSE_OTHER_MIC, &CYouMeExternDemoDlg::OnBnClickedBtnCloseOtherMic)
	ON_BN_CLICKED(IDC_BTN_OPEN_OTHER_MIC, &CYouMeExternDemoDlg::OnBnClickedBtnOpenOtherMic)
	ON_BN_CLICKED(IDC_BTN_QUIT, &CYouMeExternDemoDlg::OnBnClickedBtnQuit)
	ON_BN_CLICKED(IDC_BTN_AVOID_OTHERS, &CYouMeExternDemoDlg::OnBnClickedBtnAvoidOthers)
	ON_CBN_SELCHANGE(IDC_CMB_SERVER, &CYouMeExternDemoDlg::OnCbnSelchangeCmbServer)
	ON_BN_CLICKED(IDC_BTN_CANCLE_AVOID_OTHER, &CYouMeExternDemoDlg::OnBnClickedBtnCancleAvoidOther)
	ON_CBN_DROPDOWN(IDC_CMB_SERVER, &CYouMeExternDemoDlg::OnCbnDropdownCmbServer)
	ON_BN_CLICKED(IDC_BTN_GET_VOL, &CYouMeExternDemoDlg::OnBnClickedBtnGetVol)
	ON_MESSAGE(MEMBER_CHANGE, &CYouMeExternDemoDlg::OnMemberChange)
	ON_MESSAGE(WM_AVSTATIC_MSG, &CYouMeExternDemoDlg::OnAvStaticMsg)
	ON_MESSAGE(WM_TRANSCRIBER_END_MSG, &CYouMeExternDemoDlg::OnTranscriberMsg)
	//ON_MESSAGE(COMMON_STATUS, &CYouMeExternDemoDlg::OnCommonStatus)YouMeDemo.rc
	ON_BN_CLICKED(IDC_BTN_CLEAR, &CYouMeExternDemoDlg::OnBnClickedBtnClear)
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	ON_CBN_SELCHANGE(IDC_CMB_ENVIRONMENT, &CYouMeExternDemoDlg::OnCbnSelchangeCmbEnvironment)
END_MESSAGE_MAP()


// CYouMeExternDemoDlg 消息处理程序

BOOL CYouMeExternDemoDlg::OnInitDialog()
{
	...
	m_YouMeVideoDlg = new YouMeExternVideoDlg();
...
void CYouMeExternDemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	if ((nID & 0xFFF0) == IDM_ABOUTBOX)
	{
		CExternAboutDlg dlgAbout;
		dlgAbout.DoModal();
	}
	else
	{
...
void CYouMeExternDemoDlg::OnPaint()
{
	if (IsIconic())
	{
...
//显示。
HCURSOR CYouMeExternDemoDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CYouMeExternDemoDlg::OnBnClickedBtnLeaveroom()
{
	if (m_cConfig.EngineInstance(this->GetSafeHwnd()))
	{
		if (!isInRoom) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnJoinroom()
{
	...
	if (strUserID.IsEmpty() || strRoomID.IsEmpty())
	{
		...
		if (m_cConfig.EngineInstance(this->GetSafeHwnd()))
		{
			...
			m_cConfig.m_pEngine->setPcmCallbackEnable(m_cConfig.m_pEventCallback, pcmCallBackFlag_Remote | pcmCallBackFlag_Record | pcmCallBackFlag_Mix, true, 48000, 1);
...
YouMeErrorCode CYouMeExternDemoDlg::initEngine()
{
		
	m_cConfig.m_pEngine->setLogLevel(LOG_VERBOSE, LOG_VERBOSE);
	m_cConfig.m_pEngine->setUserLogPath(".\\ymrtc.log");
	//m_cConfig.m_pEngine->setExternalInputMode(true);
	//m_cConfig.m_pEngine->setExternalInputSampleRate(SAMPLE_RATE_48, SAMPLE_RATE_48);

	m_cConfig.m_pEngine->setAVStatisticInterval(5000);
	m_cConfig.m_pEngine->setMemberChangeCallback(m_cConfig.m_pEventCallback);
	m_cConfig.m_pEngine->setAVStatisticCallback(m_cConfig.m_pEventCallback);
...
void CYouMeExternDemoDlg::OnBnClickedBtnInit()
{
	if (m_cConfig.EngineInstance(this->GetSafeHwnd()))
	{
		if (isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnUninit()
{
...
void CYouMeExternDemoDlg::OnNMCustomdrawSliderVol(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CYouMeExternDemoDlg::OnBnClickedBtnCloseMic()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnGetMic()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnOpenMic()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnGetSpeaker()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnOpenSpeaker()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnCloseSpeaker()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnOpenOtherSpeaker()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnBnClickedBtnCloseOtherSpeaker()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnBnClickedBtnCloseOtherMic()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnBnClickedBtnOpenOtherMic()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnBnClickedBtnQuit()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.m_pEngine == NULL) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnAvoidOthers()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnBnClickedBtnCancleAvoidOther()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnCbnSelchangeCmbServer()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		CString cs;
...
void CYouMeExternDemoDlg::OnCbnDropdownCmbServer()
{
...
void CYouMeExternDemoDlg::OnBnClickedBtnGetVol()
{
	// TODO:  在此添加控件通知处理程序代码
	if (m_cConfig.EngineInstance(this->GetSafeHwnd())) {
		if (!isInit) {
 ...
 * 房间成员变化
 */
LRESULT CYouMeExternDemoDlg::OnMemberChange(WPARAM wParam, LPARAM lParam){
...
LRESULT CYouMeExternDemoDlg::OnAvStaticMsg(WPARAM wParam, LPARAM lParam){
	...
	CString strUser;
	CString strStatusDis;
	charTowchar(user, strUser);
	strStatusDis.Format(_T("回调：OnAvStaticMsg type:%s, framerate:%d"), strUser.GetString(), value);
	ShowStatus(strStatusDis);
...
LRESULT CYouMeExternDemoDlg::OnTranscriberMsg(WPARAM wParam, LPARAM lParam) {
...
afx_msg LRESULT CYouMeExternDemoDlg::OnInterfaceEvent(WPARAM wParam, LPARAM lParam)
{
 ...
 * 显示Status
 */
void CYouMeExternDemoDlg::ShowStatus(CString msg) {
...
void CYouMeExternDemoDlg::OnBnClickedBtnClear()
{
	// TODO:  在此添加控件通知处理程序代码
	m_currMessage.Empty();
	m_currStatus.SetWindowText(m_currMessage);
...
void CYouMeExternDemoDlg::OnClose()
{
...
void  CYouMeExternDemoDlg::OnVideoDlgCLose(){
	if (m_YouMeVideoDlg)
	{
		m_YouMeVideoDlg->joinRoomSuc(false);
		m_YouMeVideoDlg = NULL;
...
void CYouMeExternDemoDlg::OnCbnSelchangeCmbEnvironment()
{
	...
	//youme_setTestConfig(6);
	//youme_setServerIpPort("120.78.74.146", 9527);
}

LRESULT CYouMeExternDemoDlg::OnUninitMessage(WPARAM w, LPARAM l)
{
	YouMeErrorCode errorcode = m_cConfig.m_pEngine->unInit();
	CString strMsg;
	strMsg.Format(_T("@@@反初始化 返回值 [%d]"), errorcode);
